...
- dataset_name: LAION-5B
  ...
  citation: Schuhmann et al., 2022, NeurIPS Datasets & Benchmarks. OpenAlex 被引 1,036 (2026-06-06)
  leaderboard_url: 无官方榜(作训练语料, 下游评测见 CLIP/扩散模型 benchmark)
  known_issues: 大量链接失效(link rot)、alt-text 噪声大、CLIP 过滤引入偏差、NSFW 标注不完美、含重复
...
- dataset_name: Common Crawl
  ...
  citation: Common Crawl Foundation. 无标准学术引用, 通常引用官网或派生语料论文
  leaderboard_url: 无(原料语料, 无榜)
  known_issues: 含大量重复/低质/样板/spam, 编码错误, 语言混杂; 需重度清洗去重才可用
...
- dataset_name: The Pile
  ...
  citation: Gao et al., 2020. OpenAlex 被引 485 (2026-06-06)
  leaderboard_url: 无(预训练语料)
  known_issues: 子集去重不彻底、含基准泄漏风险、Books3 等子集已因版权下架、质量参差
...
- dataset_name: RedPajama (RedPajama-Data v1 / v2)
  ...
  citation: Weber et al., 2024, NeurIPS D&B. OpenAlex 被引 18 (2026-06-06)
  leaderboard_url: 无(预训练语料)
  known_issues: 沿袭 CommonCrawl 噪声、去重与质量过滤依赖使用者、v2 体量巨大存储/IO 压力大
...
- dataset_name: C4 (Colossal Clean Crawled Corpus)
  ...
  citation: Raffel et al., 2020, JMLR. OpenAlex 被引 3,692 (2026-06-06)
  leaderboard_url: 无(预训练语料)
  known_issues: 启发式过滤误伤(Dodge 2021 指出过滤少数群体方言/特定网站)、含基准泄漏、去重不彻底
...
- dataset_name: LAION-COCO
  ...
  citation: LAION e.V., 2022 (博客技术发布). 无标准学术引用 → 待核查
  leaderboard_url: 无(训练语料)
  known_issues: caption 为机器生成含幻觉/泛化模板、link rot、继承 LAION 噪声与过滤偏差
...
- dataset_name: WebVid (WebVid-2M / WebVid-10M)
  ...
  citation: Bain et al., 2021, ICCV. OpenAlex 被引 749 (2026-06-06)
  leaderboard_url: https://paperswithcode.com/sota/video-retrieval-on-msr-vtt
  known_issues: 视频含 Shutterstock 水印影响下游、alt-text 与视频弱对齐、来源版权致下架、link rot
...
- dataset_name: AudioCaps
  ...
  citation: Kim et al., 2019, NAACL-HLT. OpenAlex 被引 82 (2026-06-06; 经典工作实际被引更高)
  leaderboard_url: https://paperswithcode.com/sota/audio-captioning-on-audiocaps
  known_issues: 依赖 YouTube 源音频(link rot 致部分不可得)、众包描述主观/多样、与 AudioSet 标签弱一致
...
- dataset_name: MSR-VTT (Microsoft Research Video to Text)
  ...
  citation: Xu et al., 2016, CVPR. OpenAlex 被引 1,737 (2026-06-06)
  leaderboard_url: https://paperswithcode.com/sota/video-retrieval-on-msr-vtt
  known_issues: 不同论文用不同 train/test split(1k-A/1k-B 等)致结果不可直接比较、描述重复度高、规模偏小、link rot
...
- dataset_name: ScanNet (v2)
  ...
  citation: Dai et al., 2017, CVPR. OpenAlex 被引 3,990 (2026-06-06)
  leaderboard_url: http://kaldir.vc.in.tum.de/scannet_benchmark/
  known_issues: 重建网格有噪声/空洞、标注粒度不一、长尾类、消费级深度传感器误差
...
- dataset_name: KITTI
  ...
  citation: Geiger et al., 2012, CVPR. OpenAlex 被引 14,348 (2026-06-06)
  leaderboard_url: https://www.cvlibs.net/datasets/kitti/eval_object.php
  known_issues: 仅德国卡尔斯鲁厄单城晴天白天、规模小、3D 框仅前视相机视野内标注、已较饱和
...
- dataset_name: nuScenes
  ...
  citation: Caesar et al., 2020, CVPR. OpenAlex 被引 379 (2026-06-06; arXiv 1903.11027 另有计数)
  leaderboard_url: https://www.nuscenes.org/object-detection
  known_issues: 仅两城、激光线数较低(32 线)、类别长尾、标注频率 2Hz(需插值)、夜雨样本有限
...
- dataset_name: Waymo Open Dataset (Perception)
  ...
  citation: Sun et al., 2020, CVPR. OpenAlex 被引 2,998 (2026-06-06)
  leaderboard_url: https://waymo.com/open/challenges/
  known_issues: 体量大(TB 级)处理成本高、仅美国若干城市、标注/坐标系与 KITTI/nuScenes 不一致需转换
...
- dataset_name: OGB-LSC (Open Graph Benchmark — Large-Scale Challenge)
  ...
  citation: Hu et al., 2021, NeurIPS D&B. OpenAlex 被引 121 (2026-06-06)
  leaderboard_url: https://ogb.stanford.edu/docs/lsc/leaderboards/
  known_issues: 规模巨大需分布式/采样训练、内存与 IO 瓶颈、标签分布不均、3D 构象仅训练集提供(PCQM4Mv2 测试无 3D)
...
- dataset_name: ZINC (含 ZINC15 / ZINC20 与 ZINC-250k 子集)
  ...
  citation: Irwin & Shoichet, 2005, JCIM. OpenAlex 被引 4,064 (2026-06-06; ZINC15/20 另有后续论文)
  leaderboard_url: https://paperswithcode.com/sota/molecular-graph-generation-on-zinc
  known_issues: 化学空间偏类药小分子、3D 构象为计算生成、不同版本与子集规模差异大、可购状态随时间变化
...
- dataset_name: MoleculeNet
  ...
  citation: Wu et al., 2018, Chemical Science. OpenAlex 被引 2,923 (2026-06-06)
  leaderboard_url: https://paperswithcode.com/dataset/moleculenet
  known_issues: 小数据集方差大、标签噪声与缺失、类别极不均衡(如 Tox21)、scaffold split 与随机 split 结果差异显著
...
- dataset_name: PhysioNet (PhysioBank / 含 MIT-BIH, MIMIC 等)
  ...
  citation: Goldberger et al., 2000, Circulation. OpenAlex 被引 14,433 (2026-06-06)
  leaderboard_url: https://physionet.org/about/challenge/ (各年 Challenge)
  known_issues: 各库采样率/导联/标注标准不一、信号含基线漂移与工频/运动伪迹、标注者间差异、类别不均衡
...
- dataset_name: CheXpert
  ...
  citation: Irvin et al., 2019, AAAI. OpenAlex 被引 2,538 (2026-06-06)
  leaderboard_url: https://stanfordmlgroup.github.io/competitions/chexpert/
  known_issues: 标签由 NLP 从报告自动抽取(噪声)、不确定标签(-1)处理策略影响大、类别不均衡、隐藏测试集
...
- dataset_name: ISIC (HAM10000 / ISIC Archive 皮肤镜)
  ...
  citation: Tschandl et al., 2018, Scientific Data. OpenAlex 被引 3,138 (2026-06-06)
  leaderboard_url: https://challenge.isic-archive.com/leaderboards/
  known_issues: 类别极不均衡(nv 占多数)、部分标签非组织病理确诊、多源设备/标尺差异、近重复图像、毛发/气泡伪迹
...
- dataset_name: DeepWeeds
  ...
  citation: Olsen et al., 2019, Scientific Reports. OpenAlex 被引 487 (2026-06-06)
  leaderboard_url: https://paperswithcode.com/dataset/deepweeds
  known_issues: 类别不均衡(negative 类占比大)、田间光照/距离/遮挡变化大、仅澳洲特定牧场物种、背景复杂
