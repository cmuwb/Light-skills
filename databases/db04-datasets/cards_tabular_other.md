...
- dataset_name: UCI Adult (Census Income)
  ...
  known_issues: 1994 普查抽样, 时效性差; 类别不均衡 (~24% 正类); 含缺失值 (?)
...
- dataset_name: Iris
  ...
  citation: Fisher, R.A. (1936). The use of multiple measurements in taxonomic problems. Annals of Eugenics, 7(2):179-188. [OpenAlex 被引 14705]
  leaderboard_url: 无 (教学基准, 已饱和)
  known_issues: 极小且线性可分性强, 仅适合教学/快速验证; 2 类部分重叠
  bias_risk: 无
  privacy_risk: 无
...
- dataset_name: Wine (UCI)
  ...
  citation: Forina, M. et al. (1988). PARVUS - An Extendable Package for Data Exploration, Classification and Correlation. [OpenAlex 该书条目被引 132]
  leaderboard_url: 无
  known_issues: 极小样本; 特征尺度差异大 (需归一化)
  bias_risk: 无
  privacy_risk: 无
...
- dataset_name: Titanic (Kaggle)
  ...
  citation: Kaggle. Titanic - Machine Learning from Disaster.
  leaderboard_url: https://www.kaggle.com/c/titanic/leaderboard
  known_issues: 含缺失值 (Age/Cabin 大量缺失); test 标签不公开, 需提交评测
...
- dataset_name: HIGGS
  ...
  citation: Baldi, P., Sadowski, P., Whiteson, D. (2014). Searching for exotic particles in high-energy physics with deep learning. Nature Communications 5:4308. [OpenAlex 被引 1177]
  leaderboard_url: 无统一官方榜 (论文报 AUC 基准)
  known_issues: 模拟 (Monte Carlo) 数据非真实探测; 规模大, IO/内存敏感
  bias_risk: 无 (物理模拟)
  privacy_risk: 无
...
- dataset_name: OGB (Open Graph Benchmark) — ogbn-arxiv
  ...
  citation: Hu, W. et al. (2020). Open Graph Benchmark: Datasets for Machine Learning on Graphs. NeurIPS. [OpenAlex arXiv 版被引 491]
  leaderboard_url: https://ogb.stanford.edu/docs/leader_nodeprop/
  known_issues: 不同子数据集划分/指标不同; ogbn-papers100M 规模极大需分布式
  bias_risk: 学科分布不均衡
...
- dataset_name: Cora / CiteSeer / PubMed (Planetoid)
  ...
  citation: Sen, P. et al. (2008). Collective Classification in Network Data. AI Magazine 29(3). [OpenAlex 被引 3279]; 标准划分见 Yang et al. 2016 (Planetoid)
  leaderboard_url: https://paperswithcode.com/sota/node-classification-on-cora
  known_issues: 规模小易过拟合; 多种划分共存导致结果不可比; 已接近饱和
  bias_risk: 低
...
- dataset_name: Reddit (GraphSAGE)
  ...
  citation: Hamilton, W., Ying, R., Leskovec, J. (2017). Inductive Representation Learning on Large Graphs. NeurIPS. [OpenAlex 被引 4544]
  leaderboard_url: https://paperswithcode.com/sota/node-classification-on-reddit
  known_issues: 规模大需邻居采样; 不同框架预处理版本节点/边数略有差异
  bias_risk: 社区/话题分布不均
...
- dataset_name: ETT (Electricity Transformer Temperature)
  ...
  citation: Zhou, H. et al. (2021). Informer: Beyond Efficient Transformer for Long Sequence Time-Series Forecasting. AAAI (Best Paper). [OpenAlex AAAI 版被引 5981]
  leaderboard_url: https://paperswithcode.com/sota/time-series-forecasting-on-etth1-336
  known_issues: 不同论文预测长度/通道设置不一致, 比较需对齐设置
  bias_risk: 无
...
- dataset_name: M4 Competition
  ...
  citation: Makridakis, S., Spiliotis, E., Assimakopoulos, V. (2018/2020). The M4 Competition: Results, findings, conclusion and way forward. Int. J. Forecasting 34(4). [OpenAlex 被引 573]
  leaderboard_url: 待核查 (竞赛已结束, 结果见论文/官方仓库)
  known_issues: 各频率序列长度/特性差异大; 评测指标特定 (sMAPE/MASE/OWA)
...
- dataset_name: Traffic (PeMS, LSTNet 版)
  ...
  citation: Lai, G. et al. (2018). Modeling Long- and Short-Term Temporal Patterns with Deep Neural Networks (LSTNet). SIGIR. [OpenAlex 被引 2043]
  leaderboard_url: https://paperswithcode.com/sota/time-series-forecasting-on-traffic-336
  known_issues: 占用率 [0,1] 截断; 强日/周周期; 不同处理版本变量数略异
...
- dataset_name: Electricity (ElectricityLoadDiagrams, LSTNet 版)
  ...
  download_url: https://github.com/laiguokun/multivariate-time-series-data
  paper_url: https://doi.org/10.1145/3209978.3210006
  citation: 原始 Trindade, A. UCI ElectricityLoadDiagrams2011-2014; 基准用法见 Lai et al. (2018) LSTNet, SIGIR. [OpenAlex 被引 2043]
  leaderboard_url: https://paperswithcode.com/sota/time-series-forecasting-on-electricity-336
  known_issues: 原始 15 分钟级 (370 客户), 基准版重采样为小时级 321 客户, 版本需对齐
  bias_risk: 葡萄牙单一地区客户
...
- dataset_name: Flickr30k
  ...
  citation: Young, P. et al. (2014). From image descriptions to visual denotations. TACL 2. [OpenAlex 被引 2442]; Entities: Plummer et al. (2015) ICCV
  leaderboard_url: https://paperswithcode.com/sota/image-retrieval-on-flickr30k
  known_issues: 描述风格/长度不均; 标注主观性
...
- dataset_name: VQA (v2.0)
  ...
  citation: Antol, S. et al. (2015). VQA: Visual Question Answering. ICCV. [OpenAlex 被引 4274]; v2.0: Goyal et al. (2017) CVPR
  leaderboard_url: https://eval.ai/web/challenges/challenge-page/830/leaderboard
  known_issues: v1.0 语言先验偏置严重 (v2.0 平衡问答对缓解); 答案开放评分需特定指标
...
- dataset_name: AudioSet
  ...
  citation: Gemmeke, J.F. et al. (2017). Audio Set: An ontology and human-labeled dataset for audio events. ICASSP. [OpenAlex 被引 2955]
  leaderboard_url: https://paperswithcode.com/sota/audio-classification-on-audioset
  known_issues: 视频可能下架致数据漂移; 标签弱标注 (片段级, 含噪); 类别极不均衡
...
- dataset_name: MIMIC-III  ⚠ 需 credentialed access
  ...
  license: ⚠ 严格受限 — PhysioNet Credentialed Health Data License 1.5.0;
  ...
  citation: Johnson, A.E.W. et al. (2016). MIMIC-III, a freely accessible critical care database. Scientific Data 3:160035. [OpenAlex 被引 8091]
  leaderboard_url: 待核查 (benchmark 见 Harutyunyan et al. 2019 等, 无单一官方榜)
  known_issues: 单中心; 编码体系 (ICD-9) 时效; 文本含拼写/缩写噪声; 时间戳已偏移
...
- dataset_name: PlantVillage
  ...
  citation: Mohanty, S.P., Hughes, D.P., Salathé, M. (2016). Using Deep Learning for Image-Based Plant Disease Detection. Frontiers in Plant Science 7:1419. [OpenAlex 被引 4508]; 数据原始: Hughes & Salathé (2015) arXiv:1511.08060
  leaderboard_url: https://paperswithcode.com/sota/image-classification-on-plantvillage
  known_issues: 实验室受控背景, 田间泛化差; 类别不均衡; 数据泄漏风险(同叶多图)
...
- dataset_name: Sentinel-2 / BigEarthNet (遥感)
  ...
  license: Sentinel-2 影像: Copernicus 开放数据条款 (免费, 可商用/再分发, 需署名 "Contains modified Copernicus Sentinel data");
  ...
  citation: Sumbul, G. et al. (2019). BigEarthNet: A Large-Scale Benchmark Archive for Remote Sensing Image Understanding. IGARSS. [OpenAlex 被引 527]; Sentinel-2 任务: Drusch, M. et al. (2012). Sentinel-2: ESA's Optical High-Resolution Mission for GMES Operational Services. Remote Sensing of Environment 120. [OpenAlex 被引 4413]
  leaderboard_url: https://paperswithcode.com/sota/multi-label-classification-on-bigearthnet
  known_issues: 云/雪遮挡; 部分图块标签含噪 (含云者建议剔除, 见 BigEarthNet 推荐); 大气校正级别 (L1C/L2A) 需统一
