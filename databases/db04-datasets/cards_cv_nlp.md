...
- dataset_name: "ImageNet (ILSVRC)"
  ...
  citation: "Russakovsky et al., 2015, IJCV. OpenAlex 被引 40,129 (2026-06-06)"
  leaderboard_url: "https://paperswithcode.com/sota/image-classification-on-imagenet"
  known_issues: "标签噪声(约5-6%)、类别粒度不均、多目标图只给单标签、val/test 标签泄漏争议"
...
- dataset_name: "MS COCO (Common Objects in Context)"
  ...
  citation: "Lin et al., 2014, ECCV. OpenAlex 被引 42,032 (2026-06-06)"
  leaderboard_url: "https://paperswithcode.com/sota/object-detection-on-coco"
  known_issues: "小目标标注难、部分实例漏标、stuff/thing 边界模糊、caption 多样性有限"
...
- dataset_name: "PASCAL VOC (2007/2012)"
  ...
  citation: "Everingham et al., 2010, IJCV. OpenAlex 被引 19,471 (2026-06-06)"
  leaderboard_url: "https://paperswithcode.com/sota/object-detection-on-pascal-voc-2012"
  known_issues: "规模小(已被 COCO 取代)、类别少、难样本标注主观、VOC07 用 11-point AP, VOC12 用插值 AP 不一致"
...
- dataset_name: "CIFAR-10 / CIFAR-100"
  ...
  citation: "Krizhevsky, 2009, Tech Report, Univ. Toronto. OpenAlex 被引 25,499 (2026-06-06)"
  leaderboard_url: "https://paperswithcode.com/sota/image-classification-on-cifar-10"
  known_issues: "分辨率低、测试集与训练集存在近重复样本、CIFAR-100 部分细类易混; 来源 Tiny Images 母集因含冒犯内容已被撤下"
...
- dataset_name: "MNIST"
  ...
  citation: "LeCun et al., 1998, Proc. IEEE. OpenAlex 被引 57,987 (2026-06-06)"
  leaderboard_url: "https://paperswithcode.com/sota/image-classification-on-mnist"
  known_issues: "过于简单已饱和(>99.7%)、仅适合教学/快速验证、官方原始下载链接不稳定"
...
- dataset_name: "ADE20K"
  ...
  citation: "Zhou et al., 2017, CVPR. OpenAlex 被引 3,154 (2026-06-06)"
  leaderboard_url: "https://paperswithcode.com/sota/semantic-segmentation-on-ade20k"
  known_issues: "长尾类别极不均衡、标注粒度差异大、部件标注稀疏、全类评测困难(常用 150 类)"
...
- dataset_name: "Cityscapes"
  ...
  citation: "Cordts et al., 2016, CVPR. OpenAlex 被引 11,884 (2026-06-06)"
  leaderboard_url: "https://www.cityscapes-dataset.com/benchmarks/"
  known_issues: "仅德国及周边城市、天气/光照偏好晴天、粗标注噪声、夜间/恶劣天气样本少"
...
- dataset_name: "CelebA (CelebFaces Attributes)"
  ...
  citation: "Liu et al., 2015, ICCV. OpenAlex 被引 7,667 (2026-06-06)"
  leaderboard_url: "https://paperswithcode.com/dataset/celeba"
  known_issues: "名人偏差(妆容/灯光/角度非自然分布)、属性标注主观且二值化、身份标签噪声"
...
- dataset_name: "Open Images (V4–V7)"
  ...
  citation: "Kuznetsova et al., 2020, IJCV. OpenAlex 被引 1,607 (2026-06-06)"
  leaderboard_url: "https://paperswithcode.com/sota/object-detection-on-open-images-v6"
  known_issues: "标注不完全(部分类别只验证存在的框, 漏标多)、机器+人工混合标注质量不一、类别层级复杂"
...
- dataset_name: "GLUE"
  ...
  citation: "Wang et al., 2018, ICLR/EMNLP-Workshop. OpenAlex 被引 3,941 (2026-06-06)"
  leaderboard_url: "https://gluebenchmark.com/leaderboard"
  known_issues: "多任务已饱和(超人类基线), 部分任务(WNLI)训练集有陷阱、被 SuperGLUE 接棒"
...
- dataset_name: "SuperGLUE"
  ...
  citation: "Wang et al., 2019, NeurIPS. OpenAlex 被引 991 (2026-06-06)"
  leaderboard_url: "https://super.gluebenchmark.com/leaderboard"
  known_issues: "同样已被大模型刷至接近饱和、部分任务样本量小方差大、WSC 共指难"
...
- dataset_name: "SQuAD (1.1 / 2.0)"
  ...
  citation: "Rajpurkar et al., 2016, EMNLP. OpenAlex 被引 6,308 (2026-06-06); 2.0 见 Rajpurkar et al., 2018, ACL (DOI:10.18653/v1/p18-2124, 被引 2,155)"
  leaderboard_url: "https://rajpurkar.github.io/SQuAD-explorer/"
  known_issues: "答案为段落内连续 span(限制题型)、众包问题与文本词汇重叠高(浅层匹配可解)、已基本饱和"
...
- dataset_name: "WikiText (WikiText-2 / WikiText-103)"
  ...
  citation: "Merity et al., 2016/2017, ICLR. OpenAlex 被引 484 (2026-06-06)"
  leaderboard_url: "https://paperswithcode.com/sota/language-modelling-on-wikitext-103"
  known_issues: "仅维基文本(领域单一)、PPL 受词表/tokenization 影响不可跨设置直接比较、长尾词 <unk> 处理差异"
...
- dataset_name: "Common Crawl / C4 (Colossal Clean Crawled Corpus)"
  ...
  citation: "Raffel et al., 2020, JMLR (T5). OpenAlex 被引 8,337 (含各版本, 2026-06-06)"
  leaderboard_url: "无统一榜(作为预训练语料使用)"
  known_issues: "启发式清洗丢弃大量内容、含重复/低质/模板文本、毒性与偏见内容、去重不彻底、可能含基准测试泄漏"
...
- dataset_name: "IMDB (Large Movie Review Dataset)"
  ...
  citation: "Maas et al., 2011, ACL. OpenAlex 被引 3,298 (2026-06-06)"
  leaderboard_url: "https://paperswithcode.com/sota/sentiment-analysis-on-imdb"
  known_issues: "二分类去除中性(评分 5-6)分布偏极端、已饱和(>96%)、评论长度差异大"
...
- dataset_name: "CoNLL-2003 (NER)"
  ...
  citation: "Tjong Kim Sang & De Meulder, 2003, CoNLL. OpenAlex 被引 40 (该 arXiv 记录, 实际经典被引远高于此; 2026-06-06)"
  leaderboard_url: "https://paperswithcode.com/sota/named-entity-recognition-ner-on-conll-2003"
  known_issues: "实体类型少(4 类)、新闻领域窄、标注一致性问题、已接近饱和、底层文本获取门槛高"
...
- dataset_name: "WMT (Workshop/Conference on Machine Translation, 如 WMT14/16/19)"
  ...
  citation: "Bojar et al., 2014, WMT(ACL Workshop). OpenAlex 被引 537 (2026-06-06)"
  leaderboard_url: "https://paperswithcode.com/sota/machine-translation-on-wmt2014-english-german"
  known_issues: "不同论文使用的训练集/预处理/BPE 不一致导致 BLEU 不可直接比较、测试集年份各异、子语料质量参差、需统一 tokenization 与 detok 才公平"
