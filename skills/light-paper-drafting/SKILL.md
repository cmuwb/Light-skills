...
- **诚信门**：`references/integrity_gate.md`——claim 抽样配额（初稿≥30%、终稿100%）+ 引用核查动作（curl DOI/Crossref/arXiv/OpenAlex，端点已实测记 HTTP 码）。
...
## 衔接
图表交 m09/m11，引用交 m10，排版交 m12，润色交 m08，模拟审稿交 m14。版本入 db09。
