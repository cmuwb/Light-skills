...
- **机检器**：`scripts/draft_lint.py`——查残留 GAP、缺失声明、无显著性的 SOTA 句、抽取待核引用。**已重写降误报**：SOTA 措辞与显著性按**同句/相邻句窗口**共现判定（非严格同行）、**跳过 ``` 代码围栏**、必备声明按**行首 markdown 标题**校验（非全文子串）；`--claims` 抽候选事实句播种 claim 台账、`--json` 出机读结果供 orchestrator 判过门。**`--evidence <evidence_strength.json>`：消费 m06 产出的证据强度契约，机械校验正文每处断言措辞不强于其统计证据档——弱/无证据下出现 demonstrate/significantly/outperform 等强断言直接 FAIL 并给降级建议（金矿1消费端，挂接 `_shared/evidence_contract`，把"措辞不强于证据"从口号变成可运行的退出码门）。** `python scripts/draft_lint.py <draft.md> [--final] [--claims] [--json] [--evidence evidence_strength.json]`；自测 `--selftest`。人判优先，机检兜底。
