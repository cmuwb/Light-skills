...
- `scripts/thumbnail.py`：pptx→缩略图网格做视觉 QA。优先 LibreOffice 像素渲染(需 soffice + PyMuPDF/pdftoppm)；无 soffice 时自动回退纯 python(读 python-pptx 几何用 PIL 画版式示意图)，给人眼复核版式/空页/版式重复。**注意：重叠/溢出/对齐的判定不靠肉眼看示意图，由下面 `geom_qa.py` 做确定性计算**（旧版宣称"缩略图足以抓重叠/溢出"是名实不符，已改正）。`python scripts/thumbnail.py deck.pptx --cols 4`。
- `scripts/geom_qa.py`：**确定性几何 QA**（挂接共享地基契约 `_shared/visual_qa`）。读每页 shape 的 left/top/width/height(EMU)，**真计算**两两重叠/溢出画布/超安全边距/对齐偏差——补上此前"反复宣称却没做"的能力。`零重叠零溢出 = hard_pass`，a08 闸门可设为硬通过条件。有 LibreOffice/PyMuPDF 时支持 render_then_review：渲染 PNG 喂回多模态 Opus 按 `visual_qa_rubric()` 打分（无渲染器降级为仅几何检测并显式标注"未做像素级回看"）。`python scripts/geom_qa.py deck.pptx --json`；退出码 0=hard_pass/1=有重叠或溢出。
- `scripts/pptx_eval.py`：**PPT 可量化评测**(借 PPTAgent/PPTEval 思路)。把视觉 QA 从肉眼查升级为可打分、可回归——内容密度/设计一致/连贯性**+版面几何(layout)四维**扣分制(layout 维度由 geom_qa 注入确定性重叠/溢出检测，对照本节硬尺度：每页≤7 要点、字号区间、配色数、禁纯文字页/同版式)，逐页给分+扣分理由。`python scripts/pptx_eval.py deck.pptx`。**只算结构性指标(含几何)，配色品味/叙事质量/图是否真数据仍须人工 + render_then_review 把渲染图喂回多模态 Opus 复核**。
