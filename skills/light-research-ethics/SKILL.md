...
1. **投稿前**（论文/会议投稿、camera-ready 前）：跑撤稿核查(决策树 C + check_retractions.py)、相似度自查(text_overlap.py)、**全文统计自洽(extract_stats.py 扫整篇 results 段，自动抽取并重算 t/F/r/χ²/Z)**、**结论夸大门(claim_evidence_bind.py，措辞强度须等于证据强度)**、**论文工厂/机翻洗稿筛查(tortured_phrase_scan.py)**、署名/AI 披露按目标 venue 取值，登记模板。
...
## 风险审查清单
1. **学术不端**：抄袭、自我抄袭、重复发表、伪造/篡改数据、不当署名。按 ORI/42 CFR Part 93 的 FFP 口径与"显著偏离+故意/明知/轻率+优势证据"认定门槛判定，把诚实错误与学术分歧排除在外，不扣帽子（口径细节见 `references/decision_trees.md` 决策树 A）。
2. **数据造假**：编造实验结果、挑选性报告、p-hacking、隐瞒不利结果。**统计造假信号落地**：用 `stat_consistency.py` 跑 GRIM(均值粒度)与 p 值-自由度重算(t/F/r/χ²/Z 五路)、`extract_stats.py` 对整篇 results 段自动抽取报告式统计量批量重算（见下「可操作资产」）。
3. **图片不当**：图片重复使用、不当拼接/修饰、跨论文复用(对应 COPE image manipulation flowchart)。⚠ **诚实边界：本技能无图像取证代码**——图像复制/拼接/AI 生成图检测需专业工具(Proofig/Imagetwin 出版社级)或人工放大比对，裸 LLM 与本技能均无法自动检出。此项目前仅为"提请人工/专业工具核查"的清单项，不是可执行能力；高利害图像须显式走专业取证。
...
8. **结论夸大/过度包装**：声称超出证据、滥用 SOTA/novel(联动 a08/m08)。**落地工具**：`scripts/claim_evidence_bind.py` 消费 m06 的 `evidence_strength.json`(证据等级)，扫草稿强主张词(prove/demonstrate/SOTA 及中文"证明/首次/显著优于")，措辞强于证据等级即标红并给降级建议；挂接 `_shared/evidence_contract.py` 做 grade→措辞档映射，结果以 `light.findings.v1` 经 `_shared/gate_runner.py` 产出，供 a01 passport/db09 消费。无证据文件时降级为"无支撑强主张"全标。
9. **论文代写**：不代写以欺骗为目的的内容；辅助应是协作而非替考/造假。
10. **专利权属**：发明人/权利人认定，职务发明，最终文本须代理人审核(联动 m15)。
11. **软著真实性**：软件须真实存在、材料不虚构(联动 m15)。
12. **论文工厂/机翻洗稿**：草稿或待引文献含"扭曲短语"(tortured phrase，如 "artificial neural organization"=neural network、"bosom peril"=breast cancer)是论文工厂/机器洗稿的高发指纹。`scripts/tortured_phrase_scan.py` 用 `references/tortured_phrases.json` 词典子串筛查草稿与参考文献标题/摘要，命中即登记红旗(需人工复核+联系作者，非定罪)；词典可增长。
...
- **相似度/AI 检测**：文本匹配≠抄袭，先排除引文/参考文献/小段匹配，再看"大段未标注"。AI 检测分数只作"对话起点"非定罪依据，对非母语写作误报偏高，须人工复核+与作者沟通，不作高利害判定唯一依据（厂商自报误报率等具体数字属可检索事实，引用前回查最新页，见 references.md）。**自查重落地**：Turnitin 本体闭源不可复刻，但本地自我抄袭/重复发表筛查可做——用 `scripts/text_overlap.py` 把目标文与自己旧论文语料比对，找最长逐字连续重合(对应红旗 >40 词)，仅限所给语料、不得对外宣称相似度%(见 references.md「离线自查重」)。
- **代码/许可/供应链**：许可+SBOM(ScanCode)、漏洞(Snyk)、恶意包行为(Socket)分工互补；具体命令/参数见 `references.md`。
...
- **统计自洽快查脚本** `scripts/stat_consistency.py`：把"统计量与自由度不自洽""均值粒度不可能"两类高频造假信号落地为可跑工具(纯 stdlib)——GRIM 检验、granularity 粒度检验、p值-自由度一致性快查。**p 值重算覆盖 t/F/r/χ²/Z 五路**(纯 stdlib 自带正则不完全 Beta + 不完全 Gamma + erfc 尾函数，对标 statcheck 全覆盖)。`--json`/`--selftest` 离线自测。诚实限制:GRIM/粒度只对整数项目有效，连续测量不适用；p 值快查只查量级级别的明显矛盾，非精确复算，结果是"需人工复核的信号"非定罪。
- **全文统计抽取脚本** `scripts/extract_stats.py`：statcheck 式落地——正则扫整篇手稿/results 段，自动抽出所有报告式 NHST 结果(t(df)、F(df1,df2)、r(df)、χ²(df,N)、Z 配 p 值与比较符)，逐条调 stat_consistency.pcheck 重算，产"报告 p vs 重算 p"全表，GROSS 标红。`--json`/`--selftest`。诚实限制:正则抽取尽力而为，漏抽是静默的(非"已查无问题")，表格内/跨行/非标准排版可能漏；重算是量级 sanity check 非定罪。跨技能:抽取结果可回喂 m07 在投稿前对 results 段统计量做拦截。
- **结论夸大门脚本** `scripts/claim_evidence_bind.py`：给清单第8条装上可执行工具——消费 m06 `evidence_strength.json`(证据等级)，扫草稿强主张词(prove/demonstrate/SOTA/novel 及中文"证明/首次/显著优于")，措辞强于证据等级即标红+给降级措辞。**挂接两个 _shared 契约**:用 `evidence_contract.py` 的 grade→措辞档映射(grade_evidence/allowed_verb_tier/lint_wording)，结果经 `gate_runner.py` 产 `light.findings.v1` 报告供 a01 passport/db09 消费(机读 verdict 非 prose)。无证据文件时降级为"无支撑强主张"全标。`--json`/`--selftest`。
- **论文工厂筛查脚本** `scripts/tortured_phrase_scan.py`：扫草稿与待引文献标题/摘要的"扭曲短语"(机翻/洗稿指纹)，命中即报疑似论文工厂红旗。词典 `references/tortured_phrases.json` 可增长，缺文件回退内联种子集。跨技能反哺:`--refs` 传参考文献列表，供 m10 light-citation 引用核验时连带做 paper-mill 预警。`--json`/`--selftest`。诚实限制:命中是"需人工复核"信号非定罪，词典有限、无命中不等于真实可靠。
...
**跨技能契约**：消费 m06 `evidence_strength.json`(结论夸大门的证据等级源)；`claim_evidence_bind.py` 产 `light.findings.v1` findings 报告(经 `_shared/gate_runner.py`)，供 a01 passport 确认点判定与 db09 记忆机读消费；撤稿标记类型 `references/retraction_flag_types.json` 与 m10 `verify_refs.py` 同源(改一处同步两处)；`tortured_phrase_scan.py --refs` 供 m10 引用核验时连带 paper-mill 预警。
