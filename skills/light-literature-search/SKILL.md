...
- **生医纪律（必做）**：涉生医/临床/系统综述方向，**PubMed+MeSH 检索式为必做项**(对齐已宣称的 PRISMA)，不能只靠 OpenAlex/S2 的全文检索冒充规范检索；MeSH 受控词与 Clinical Queries 临床过滤器是 PubMed 独有能力。Europe PMC 作免 key 补充(直接给 abstract+开放获取标记+引用端点)。
