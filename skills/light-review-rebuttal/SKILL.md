...
> **增量边界（诚实，别把裸模型自带的评审常识当本技能贡献）**：NeurIPS 评审表字段、ScholarEval 8 维 rubric、返修原则（每条都回/不对抗但坚持/能补就补）、R→A→C 格式、GRADE/Cochrane 清单——都是**强 Opus 自带的召回，近零增量**。本技能真正超出裸模型的是：①`check_commitments.py` 把承诺账本/COMMITMENT_GAP/连续让步从手填勾选变**真退出码门**（审查发现的最大名实差距）②`fetch_openreview.py` 的 OpenReview per-submission invitation 坑实测规避（裸模型必照抄 venue 级空端点）③`rebuttal_budget.py` venue 字数预算+CJK 分词④反谄媚两段式 concession 证据门禁（裸 LLM rebuttal 出名地过度妥协，这套行为协议真改输出姿态）。**诚实落后项**（未做）：DRPG 的检索式 concern→段落 grounding（response_matrix 的"改动位置"仍人手填）、RbtAct 的 actionability 影响分级、把 OpenReview rating 分布回灌模拟评审打分的校准闭环。
...
5. **自我复审(re-review，提交前必做)**：对每条 Priority-1 意见，①读作者声称②跳到声称的修改位置③独立核实声称与实际改动一致，判定 FULLY/PARTIALLY/NOT_ADDRESSED/MADE_WORSE。作者声称为空或含糊("已按建议修改")即标 🔍 无法核实，打回重写。承诺账本里非 fulfilled 项必须带 rationale，否则报 COMMITMENT_GAP。
   - **COMMITMENT_GAP 现在是真退出码门，不再是手填勾选（补本技能最大名实差距）**：填完 `response_matrix.md` 跑 `python scripts/check_commitments.py --matrix response_matrix.md`，机械核验——『已改可核/已兑现』但"改动位置"无具体定位锚（§/Table/Fig/节/行）即报 `COMMITMENT_GAP`（critical 退出码 1，拦住"空口已改"裸模型高发幻觉）；标已兑现却含未来词（将/拟/待补）报 `PLANNED_AS_DONE`；concession≥4 无新证据报 `CONCESSION_NO_EVIDENCE`、相邻连续让步报 `CONSECUTIVE_CONCESSION`（反谄媚）；major 意见回应+改动都空报 `MAJOR_UNADDRESSED`。出 `light.findings.v1` 接 a08/orchestrator。**有 `--evidence-map {意见ID:工件路径}` 时还核工件真存在**（对齐 Review2Rebuttal 的 author-approvals 工件级门禁），缺失则主张降级 planned 待签。⚠ 核账本自洽性与定位锚，不替你判改动对错。
