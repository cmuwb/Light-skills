...
- **导出**：`scripts/figure_export.py`
  ...
  - `check_scaled_fonts(fig, journal=, column=)` 校验「大尺寸画再缩小到栏宽」后的**有效字号**是否仍达下限——顶刊常先按大画布作图再缩到单栏，缩放后字号会等比变小，这个盲点 check_figure_size 抓不到。
...
> **custom 逃生通道**：尚未进上表的刊（如中文刊）用 `save_for_journal(fig, base, journal="custom", custom_width_mm=84.0)` 直接传物理栏宽（mm），校验同理 `check_figure_size(fig, journal="custom", custom_width_mm=84.0)`。**栏宽数据须有来源（db01 卡或实测记录），禁止臆测**；R3 会把中文刊实测栏宽补进 db01。
...
- **导出后必检**：每张图导出后运行 `check_figure_size(fig, journal=target_journal, column=column)` 与 `check_scaled_fonts(fig, journal=target_journal, column=column)`；若代码图涉及误差棒、截断轴、双 y 轴、jet/rainbow 等，再用 `scripts/figure_integrity_lint.py --file <plot.py>` 做诚实性检查。
