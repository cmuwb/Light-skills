...
> **增量边界（诚实，别把裸模型自带的绘图常识当本技能贡献）**：工具选择大全（matplotlib/R/MATLAB/Illustrator…各自调用）、"色盲友好用 viridis 避免 jet""矢量优先 PDF/SVG""坐标轴标签+单位""caption 自洽""去 chart junk""子图标 (a)(b)(c)"——都是**裸 Opus 全会的绘图常识，近零增量**，占了 SKILL 近半篇幅是参考资料非能力，且脚本真正落地的只有 matplotlib/seaborn 一条链（已诚实披露）。本技能真正超出裸模型与单体竞品（SciencePlots/cnsplots/同名 Claude skill）的是**导出后还能核验的验证层 + 跨技能工件契约**：①`figure_visual_qa.py` 渲染后多模态回看（全市场盲区，竞品都不看图）②`figure_integrity_lint.py` 自动诚实性扫描（偷偷截断/双轴伪相关/无误差棒，竞品均无）③`check_figure_size(measured=True)` 导出后读回真实物理宽度④figure manifest 跨技能图↔图号↔caption 契约。**诚实落后项**（未做）：cnsplots 式自动统计标注引擎（lint 只会催"没标误差类型"却不会算不会标）、pub-ready-plots/rsmf 的 LaTeX 真实单位精确尺寸（m11 按名义 mm 定尺寸）、pylustrator 式交互精修回写代码。
...
- **导出后必做·视觉回看（本技能相对全市场最大增量，别只靠盲代码 lint）**：上面三检（size/compliance/integrity）都是静态/几何校验，**抓不到任何视觉版面问题**。`scripts/figure_visual_qa.py`（挂 `_shared/visual_qa`）补这块：①从 figure 抽文本/图例 AABB 做几何检测（标签重叠、文字溢出被裁、跨类错位）；②按 `target_journal/column` 真实栏宽渲染 PNG，用 `build_review_request()` 把图 + rubric 打包，**你（多模态 Opus）必须真看一眼这张 PNG**，逐维打分并列缺陷（标签重叠/图例压数据/刻度挤/子图错位/缩印后是否可读），把返回 issues 作 `vlm_findings` 回灌 `run_geometry_qa()`，得到含像素级回看的 `light.visual_qa.v1`，写进 manifest `checks.visual`。任一 critical → 改图重渲染重看，直到无 critical（fix-and-verify 循环）。**未喂回模型时 `pixel_review_done=False` 诚实标注"未做像素级回看"，绝不假装已验证。**
...
- **figure manifest**：交付 m07 的单一工件，把"图文件↔图号↔caption"绑定。逐图一项，含：`figure_id`(沿用 m09 规划卡的 F#/T# 编号，与 m07 模板 `[图位 F1]`/`[表位 T1]` 占位对齐) + `source_card` + 图片文件路径(矢量+位图) + 最终 caption + 放置章节 + target_journal/column + 导出/字号检查结果 + **`checks.visual`（`figure_visual_qa.py` 的视觉回看结果：verdict + pixel_review_done + issues）**。m07 据此把占位替换为实图与图号，无需回查规划卡。
  示例：
  ```yaml
  - figure_id: F1
