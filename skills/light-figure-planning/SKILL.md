...
5. **display item 预算核查 + 反冗余（机制化，别只靠人脑——这是本技能着墨最重的卖点，现已落成脚本门禁）**：所有规划卡填好后跑 `python scripts/audit_figure_set.py 卡1.md 卡2.md ... --cap <目标刊上限>`：①统计正文 F#+T# 总数对照 venue 上限（顶刊/顶会常 **6–8 件**，**须以目标刊作者指南权威值传 --cap**，脚本不内嵌某刊上限臆测），**超预算按砍序自动给可执行裁定**：先砍"可删"→"可做"降附录→"必做"不动（删了没核心论证的不许砍）；②**反冗余**：自动检测"两卡绑同一 claim（`_shared/semantic_sim` 语义近似，非词面）且 figure_type 同族"的候选冗余 panel——把"遮住一个 panel 丢独有信息"落成 claim 重叠×图型族重叠的机检。出 `light.findings.v1` 接 a08 闸门，与 `validate_plan_card.py` 一起作交 m11 前**双门禁**。⚠ 冗余是候选提示（同 claim 同族也可能是有意的 overview→deviation 递进），最终合并/删除须人判。完整示例见 [examples/worked_example.md](examples/worked_example.md)。
...
- 投稿前 14 点核查：分辨率·格式·尺寸·可读性·色盲安全·灰度兼容·坐标轴标签(含单位)·误差棒·panel 标号·显著性标记·图例清晰·字体一致·去装饰元素·**display item 总数 ≤ 目标 venue 上限**（由 `audit_figure_set.py --cap` 机检 F#+T# 计数，超则按"可删→可做降附录"取舍）。

## 交 m11 前：双门禁（可机检，前移打回）
规划卡交 m11 执行**前**过两道机检门，把 m11 会打回 / 审稿人会嫌的问题拦在规划阶段：
- **门1 单卡契约** `scripts/validate_plan_card.py <卡.md>...`：校验 `target_journal` 命中 figure_export.py `JOURNAL_SPECS` 键（或 custom 带 `custom_width_mm`）、`column` 是该刊实有档位、`figure_id` 形如 F#/T# 且批量唯一（与 m07 占位对齐）、`source_card` 必填。脚本**动态从 m11 的 figure_export.py 读 JOURNAL_SPECS**（单一真相源，杜绝栏宽键漂移）；导入不到回退内置快照并标 `[SNAPSHOT]`。
- **门2 集合级审计** `scripts/audit_figure_set.py <卡...> --cap <venue上限>`：display item 预算砍序 + 反冗余 panel 检测（见规划流程第 5 步）。
- 两门全过（exit 0）再交 m11；有 ✗ 先按提示补卡/砍件/去冗余。两门只校验可机检的契约（键/计数/重叠），不替代图本身的质量判断。

> **增量边界（诚实，别把裸模型自带的图表观当本技能贡献）**：图表类型清单、工具选型速查、"图表服务论点不是装饰""caption 独立可读/误差棒+n+显著性/Okabe-Ito 色盲安全"——都是**裸 Opus 全知道的通行常识，近零增量**，是参考资料非能力。本技能真正超出裸模型的是**可机检的跨卡管线门禁**：①`audit_figure_set.py` 集合级预算砍序+反冗余（SKILL 着墨最重、此前纯人脑的卖点，现机制化）②`validate_plan_card.py` 单卡契约对齐 m11 执行层（栏宽键/figure_id 唯一性）③db07 figure-card 按 research_field 受控取值防偏科误推。**诚实落后项**（未做）：Crafter/PlotGen 式渲染后 critic 看图迭代（规划态不渲染，回看属 m11）、Draco 式 ASP 约束求解+可学习权重图型推荐（recommend_chart 是固定手调权重非学习）、m06 证据强度自动 seeding 图型与优先级——这些是对标 SOTA 的已知短板。
