...
> **增量边界（诚实，别把裸模型自带的写作常识当本技能贡献）**：审稿人五问（soundness/novelty/clarity…）、Claim-Evidence-Boundary 三件套、Hedging 阶梯、四栏输出格式、引言四段式、摘要 150 词套路、AI 使用披露模板——都是**裸 Opus 自带的写作/评审常识，近零增量**，占了近半篇幅。本技能真正超出裸模型的是**确定性可复现的工具**：①`contribution_consistency.py` 把"贡献三处一致"招牌 claim 工具化（裸模型长稿最易漏的不一致）②`mechanical_check.py` 逐行可复现扫描 + `--evidence-map` 措辞↔证据双向校准（绑 m06）③`polish.py` 对 LanguageTool 的工程化封装（分块/退避/offset 回映/真实 HTTP 码不幻觉）④`style_fingerprint.py` 对作者本人语料的定量文风偏离（反同质化）。**诚实落后项**（未做）：bahayonghang 的 latexmk/chktex 真实编译门禁（m08 对 .tex 只正则 strip 不真编译）、Writefull 的百万期刊语料惯用句式正向建议（m08 只能 flag 偏离不能给地道措辞）、润色完整性守卫（润色篡改数字/引用的 diff guard 尚未做）。
...
- **`scripts/mechanical_check.py`**（**无需 API**）：扫 ① overclaim 黑名单（significant/seminal/novel/outperforms…，**统计语境的 significant 已上下文豁免不误报**）② AI 腔/填充语 ③ hedge 堆叠 ④ **claim_strength**：强主张词（prove/conclusively/unprecedented/always…）给 Hedging 阶梯降级建议（对接 `references/argument_review.md` §2，已纳入 schema §3/§4）⑤ 段落被动句占比超阈值 ⑥ 标点卫生。**已修工程缺陷**：`--latex` 先 strip_latex 去 \\cite/$...$/注释/environment 再查（保行号，治 .tex 误报）；句切分加缩写保护（et al./e.g./Fig. 不误断）；**中文稿支撑**中文夸大词/AI 腔词表（显著/大幅提升/综上所述…）。输出 `{line,col,category,issue,suggestion,context}`。纯 stdlib，任何环境可跑。
  - **措辞强度↔证据强度双向校准（跨技能反哺，挂 `_shared/semantic_sim` + m06 证据卡）**：`--evidence-map evidence_strength.json` 把 m06 判 **strong** 档的 claim 语义定位到草稿句子——命中其中的强主张词**豁免降级**（强证据配强词是对的，治"上下文无关词表对真有数学证明/大样本的强主张误报"）；强证据 span 内若堆叠 hedge 反而提示 **under_claim**（治"确定结果写得心虚 may possibly suggest"）。把 Hedging 阶梯从单向降级升级为双向校准，兑现 prompt 的"m06 证据强度绑定措辞"。
  - 跑法：`python scripts/mechanical_check.py --file paper.txt --json`；`.tex` 自动或显式 `--latex`；绑证据 `--evidence-map evidence_strength.json`。
- **`scripts/contribution_consistency.py`**（**无需 API**，挂 `_shared/semantic_sim`/`findings_schema`）：**把"贡献三处一致"这个招牌 claim 工具化**（此前全靠 LLM 裸眼，是 m08 最大 claim-vs-code 缺口）。抽 abstract/introduction/conclusion 三处贡献句跨章节语义匹配，flag **NUMBER-DRIFT**（某处带量化指标 3.1 points 而另一处缺失/数字不同）/ **STRENGTH-DRIFT**（断言强度跨章节差≥1 档）/ **COVERAGE-DRIFT**（abstract 声明的贡献 conclusion 找不到对应=虎头蛇尾）。三处贡献分散首尾、裸模型长稿最易遗漏不一致，机检后从"正确的废话"变真增量。drift 产物可喂 m07 重构对齐措辞、m14 预审（贡献不一致=Significance 意见）。跑法：`python scripts/contribution_consistency.py --in draft.md --json`。⚠ 离线档对纯同义改写匹配偏弱（真用注入 embedding 档）。
