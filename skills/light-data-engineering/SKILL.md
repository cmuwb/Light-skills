...
- 数据增强：按模态选择（图像/文本/时序/表格 SMOTE 等）。
...
## 自建数据集规划
采集方式、标注规范(标注指南+一致性 κ 检验)、数据格式、数据说明文档(datasheet)、隐私合规(脱敏/授权/许可)、发布方式(Zenodo/HF/Figshare + DOI + license)。产出 dataset_card（见 db04 字段）。
...
- `assets/data_card_template.md`：datasheet 模板（字段对齐 db04），含质量评估、偏差/隐私风险、推荐划分、溯源核实区。
