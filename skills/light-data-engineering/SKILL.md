...
- 数据增强：按模态选工具，**先划分再增强、只增训练折**（验证/测试保持原始分布，否则指标虚高）——图像 albumentations（bbox/mask 同步变换）、文本 nlpaug（同义词/上下文/回译，注意别翻转标签语义）、时序 tsaug（TimeWarp/Drift/加噪，注意别破坏因果引入泄漏）、表格 SMOTE 等只在训练折拟合。各模态用法与红线见 references「数据增强」节。**"只增训练折"这条最易踩的泄漏线，增强后用 `scripts/split_leakage.py` 复查训练/测试是否串了同一/近似样本（增强若误作用于全量，会把同一原始样本的多个变体撒进两侧）。**
...
- **划分后审计泄漏（三道护栏分工，缺一不可）**：`safe_split.py` 防 **fit 穿越**（预处理在划分前对全量拟合）、`drift_check.py` 比 **分布**（train/test 是否同分布）、**`scripts/split_leakage.py` 查 train/test 是否串了 同一/近似样本或同一实体**——这是前两者都发现不了的一类污染（原始数据有重复行被分到两侧 / 目标均值编码用了全量算）。`LEAK-02`(无跨split样本重叠) / `SPLIT-02`(无实体重叠)：精确重复+实体重叠为 HIGH→退出码 1，可当回边/CI 闸门；近重复为 MED 需人工复核。findings.json 经 emit_artifacts 登记供 a07 回扫、a04 复核数据声明。
...
- `scripts/split_leakage.py`：**train/test 行级·实体级·近重复 泄漏审计**（`LEAK-02`/`SPLIT-02`，补 safe_split 防 fit 穿越、drift_check 比分布之外的"同一/近似样本串进两侧"这类污染）。四查：(a) 跨 split **精确重复行**（特征全同→HIGH）；(b) 数值列分箱+类别原值指纹的**近重复**（MED，排除精确重复，可能巧合）；(c) `--group-col` **实体重叠**（同一用户/患者/牧场跨 split→HIGH，复用 SPLIT-02）；(d) `--target` **目标均值编码穿越**（某数值特征在某类别列各水平内≈用全量含 test 算的目标均值→HIGH/MED）。两种输入：`--train+--test` 或 `--csv+--split-col`（支持 ≥2 split）。产 `leak_audit.md`（HIGH/MED 分级）+ `findings.json`（挂 `_shared/findings_schema` 的 `light.findings.v1`，契约不可用则优雅降级裸列表，绝不静默假成功）。**任一 HIGH→退出码 1**，可当回边/CI 闸门。纯 pandas/numpy。
  - 自测：`python scripts/split_leakage.py --selftest`（造"10 行同时塞两 split + 实体重叠 + 目标编码列"验证全命中，并验干净数据零 HIGH 误报）。
  - 用法：`python scripts/split_leakage.py --train train.csv --test test.csv --group-col user_id --target y --out leak_audit.md --findings leak_findings.json`；单文件 `--csv data.csv --split-col split`。findings.json 经 `emit_artifacts.py --register --leak-findings` 登记进 passport 供 a07 回扫。
