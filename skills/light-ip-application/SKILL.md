...
> **增量边界（诚实，别把审查指南常识当本技能贡献）**：说明书四段式结构、"须充分公开/以说明书为依据/术语前后一致"、软著流程周期、查新七系统选用表——都是**裸 Opus 直接能复述的审查指南常识，近零增量**，模板只是把它固化供套用。本技能真正超出裸模型的是**可计算、长文档里人脑最易漏的校验**：①`claim_lint.py` 权利要求一致性 linter（antecedent basis/word-support/编号方向/混用/图-标记，对标 Patent Bots 核心）②`copyright_source_prep.py` 克制可信的脱敏（明示"仅正则粗筛不保证完备"不给彻底脱敏错觉）+ 有效代码行数口径③`patent_search.py` 免凭证查新 URL + 引用图一跳雪球。**诚实落后项**（未做、不假装）：Tree-of-Claims 式审查员模拟+MCTS 迭代改写闭环、权利要求×在先技术披露逐特征映射、论文→交底书自动抽取——这些需 LLM 判定/跨技能管线，当前靠人工+agent，是对标 SOTA 的已知短板而非已实现。
...
3. **权利要求书草案**：
   ...
   - **草案成文后必跑一致性 linter（这是本技能相对裸模型/Patent Bots 的核心增量，别只口头强调"术语一致"）**：`python scripts/claim_lint.py --claims ip/claims_draft.md --spec ip/specification_draft.md`，规则化查五类形式硬错——①从属项引用方向（不得引用≥自身编号、多项须用"或"）②**antecedent basis**（出现"所述/该 X"但前文未先"一种 X"引入=缺在先基础）③**word-support**（权利要求术语未在说明书出现=缺支持）④单部分/两部分法混用（"包括以下步骤"骨架里混入"其特征在于"=代理师直接打回）⑤图-标记悬空（说明书引用的附图标记号在标记清单无定义）。major 问题退出码 1、出 `light.findings.v1` 接 a08 闸门（落 `ip/claim_lint_report.md`）。⚠ 纯正则抓形式硬错，**不替代代理师对实质内容的审核**。
...
- `scripts/claim_lint.py`：**权利要求一致性 linter（规则版，本技能核心增量，对标 Patent Bots 的 numbering/antecedent basis/word-support 三类检测）**。查从属项引用方向 + antecedent basis 在先基础 + word-support 说明书支持 + 单部分/两部分法混用 + 图-标记悬空五类形式硬错，出 `light.findings.v1`（major→退出码 1）接 a08 闸门。纯正则零依赖、不联网。
  - `python scripts/claim_lint.py --claims ip/claims_draft.md --spec ip/specification_draft.md`
  - `python scripts/claim_lint.py --claims ip/claims_draft.md --json`（输出 findings JSON）
  - `python scripts/claim_lint.py --selftest`（离线合成样本自测）
...
专利：可专利性分析 + 交底书（`disclosure_template.md`）+ 权利要求草案（`claims_template.md`）+ 说明书草案（`specification_template.md`）+ 检索证据（`patent_search.py`）+ **权利要求一致性检查报告 `ip/claim_lint_report.md`（`claim_lint.py`）** + 附图清单 + "需代理人审核"标注。
...
## 衔接
技术内容取自 m05/a03/a04/m07；软著功能说明与专利背景技术/发明内容可复用论文(m07)对应段落——专利的"要解决的技术问题/有益效果"与论文 Introduction/Results 高度同源，软著功能说明亦可借论文方法与结果段。附图交 m11（专利附图规范见 m11 references「专利附图规范」节，黑白线条+图号标记规范）；与论文/系统保持一致(a07)；材料入 db09。检索与权属风险上报 a10。交付前过 a08(light-self-review)自检闸门——**权利要求草案必先过 `claim_lint.py` 无 major 形式硬错**（其 `light.findings.v1` 接 a08/orchestrator 闸门）。
