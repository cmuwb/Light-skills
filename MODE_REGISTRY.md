...
> 说明：这张表故意只收录**真正存在显式 mode 机制**的技能。Light 共 28 个技能，但有显式 mode 的仅 3 个、合计 9 个 mode。其余技能走固定流程，不在此列——这是 Light 与"少技能多模式"型技能包（如 ARS）的结构差异，不是遗漏。
...
| `self-review` | 逐维问题清单 + 命中失败模式 + 必改项 + 声明缺失项 | "帮我审一下 / 挑毛病 / 模拟审稿" |
